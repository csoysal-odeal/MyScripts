...
SELECT bmt.serial_no, bmt.tid, bmt.mid, bmt.bank_name, bmt.merchant_name
FROM paymatch.bank_mid_tid bmt
WHERE bmt.serial_no = "2C51362498"

SELECT * FROM odeal.TerminalMerchantMappingHistory tmmh

SELECT o.id, o.unvan, o.address, c.name as Il, t.name as Ilce, IF(o.isActivated=1,"Aktif","Pasif") as UyeDurum,
       o.activatedAt, o.deActivatedAt, MIN(bp.signedDate) as IlkIslemTarih, MAX(bp.signedDate) as SonIslemTarih, COUNT(bp.id) as IslemAdet, SUM(bp.amount) as IslemTutar
FROM odeal.Organisation o
                 JOIN odeal.BasePayment bp ON bp.organisationId = o.id AND bp.currentStatus = 6 AND bp.paymentType <> 4
                JOIN odeal.City c ON c.id = o.cityId
